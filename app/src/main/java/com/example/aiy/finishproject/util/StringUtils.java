...
public class StringUtils {
    ...
    public static String getFirstImg(Context context,String htmlText){
        List<String> list= StringUtils.getTextFromHtml(htmlText,true);
        if (list.size()==0)
            return NoteManager.HTML_NO_HAS_IMG;
        LoggerUtils.i(list.get(0));
        LoggerUtils.i(FileUtils.getImageAbsolutePath(context,Uri.parse(list.get(0))));
        return  FileUtils.getImageAbsolutePath(context,Uri.parse(list.get(0)));
    }

    public static String getIntroductionText(String htmlText){
        List<String> list= StringUtils.getTextFromHtml(htmlText,false);
        StringBuilder stringBuilder=new StringBuilder();
        for (int i=0;i<list.size();i++){
            if (stringBuilder.length()>15) break;
            String s=list.get(i);
            if (s.length()>20)
                stringBuilder.append(s.substring(0,20));
            else
